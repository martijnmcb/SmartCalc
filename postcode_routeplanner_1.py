    ...
    if "batch_df_result" in st.session_state:
        ...
        if selected_id is not None:
            with st.spinner("Genereer kaarten..."):
                ...
                # Fallback: waarschuw als route_orig/route_opt niet beschikbaar zijn
                if route_orig is None or route_opt is None:
                    st.warning("⚠️ Routedata niet beschikbaar voor deze batch. Voer de batchverwerking opnieuw uit of controleer de invoer.")

                # Toon gecombineerde kaart met beide routes (origineel = rood, geoptimaliseerd = groen)
                if (route_orig or route_opt) and (route_orig is not None and route_opt is not None):
