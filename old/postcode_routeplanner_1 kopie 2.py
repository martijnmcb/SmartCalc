...
def get_osrm_route(coords_list):
    ...
    url = f"http://localhost:5000/route/v1/driving/{coord_string}?overview=full&geometries=geojson&steps=true"
    ...
    if "batch_df_result" in st.session_state:
        ...
        with col1:
            if route_orig:
                ...
                st_folium(m1, height=400, key="map_excel")
        ...
        with col2:
            if route_opt:
                ...
                st_folium(m2, height=400, key="map_optimaal")

        # Klokdiagram tonen na kaarten
        # Vraag de volledige geoptimaliseerde route opnieuw op met OSRM (inclusief legs)
        full_osrm_opt = get_osrm_route(coords_opt)
        if full_osrm_opt and "legs" in full_osrm_opt["routes"][0]:
            ...
            # st.subheader("🕒 Klokdiagram geoptimaliseerde route")
            # plot_klokdiagram(tijden, labels)
            st.subheader("📅 Gantt chart geoptimaliseerde route")
            plot_gantt_chart(tijden, labels)
